...
if (data) {
    ...
    text.innerHTML = data
        .map((elem) => `<span id=id_${elem.id}> ${elem.count} - ${elem.text}</span> </br>`)
        .join("");
    if (performed != null) {
...
document.body.addEventListener("click", (e) => {
    if (e.target.matches(".add")) {
        ...
        if (action__text) {
            ...
            new_elem.innerHTML =
                `<span id=id-${i} >` + count + " - " + action__text + `</span> ` + "<br>";
...
document.body.addEventListener("click", (e) => {
    if (e.target.matches(".eraser") || e.target.matches(".remove")) {
        ...
        document.querySelector("#text").addEventListener("click", function listener(e) {
            ...
            document.querySelector("#text").removeEventListener("click", listener, false);
        });
    }
});